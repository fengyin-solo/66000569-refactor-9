<template>
  <div class="panel">
    <h4>📜 执行日志</h4>
    <div class="log-list">
      <div v-for="(l,i) in logs" :key="i" class="log-row" :class="(resultDef(l.status)?.className ?? l.status.toLowerCase())"
        :style="{ color: resultDef(l.status)?.logColor, background: resultDef(l.status)?.logBackground }">
